 ...
 * Redireciona para /api/tiktok-oauth.php?action=callback
 */
...
$ch = curl_init();
$callbackUrl = 'http://' . $_SERVER['HTTP_HOST'] . '/api/tiktok-oauth.php?action=callback&code=' . urlencode($code) . '&state=' . urlencode($state);
...
if ($data['success']) {
    ...
    echo "<p><a href='/' style='background: #00f7ef; color: #000; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>Voltar ao Dashboard</a></p>";
    echo "</div>";
} else {
    echo "<div style='font-family: Arial, sans-serif; padding: 20px; text-align: center;'>";
    echo "<h1 style='color: #ff4757;'>❌ Erro na Autenticação</h1>";
    echo "<p>" . htmlspecialchars($data['message']) . "</p>";
    echo "<p><a href='/' style='background: #ff4757; color: #fff; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>Voltar ao Dashboard</a></p>";
    echo "</div>";
}
?>
